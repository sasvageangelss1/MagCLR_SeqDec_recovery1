...
AGGREGATED_CSV = PROJECT_ROOT / "thesis_paper_output2" / "aggregated_results.csv"
SOURCE_CSV_DIR = PROJECT_ROOT / "experiments" / "data_result" / "csv"
OUTPUT_FIGURES = PROJECT_ROOT / "thesis_paper_output2" / "figures"
OUTPUT_TABLES = PROJECT_ROOT / "thesis_paper_output2" / "tables"
...
def style_legends(axes, loc="best", ncol=1, fontsize=FS_LEGEND):
    """为一组子图分别添加图例。

    主要用于左右两幅 CDF 子图，避免图例只出现在左侧子图中。
    """
    for ax in np.ravel(axes):
        style_legend(ax, loc=loc, ncol=ncol, fontsize=fontsize)
...
def generate_figure_3_7_2_backbone_cdf(raw_df: pd.DataFrame):
    ...
    style_legends(axes, loc="lower right", fontsize=8.0)
...
def generate_figure_3_7_3_pretrain_cdf(raw_df: pd.DataFrame):
    ...
    style_legends(axes, loc="lower right")
...
def generate_figure_3_7_5_downstream_cdf(raw_df: pd.DataFrame):
    ...
    style_legends(axes, loc="lower right", fontsize=7.6)
...
def generate_figure_4_7_3_continuous_cdf(raw_df: pd.DataFrame):
    ...
    style_legends(axes, loc="lower right", fontsize=7.5)
